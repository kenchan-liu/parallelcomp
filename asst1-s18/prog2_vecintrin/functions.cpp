...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
    ...
    for (int i = 0; i < N; i += VECTOR_WIDTH) {
        ...
        _cmu418_vload_float(x, values+i, maskAll); // x = values[i]
        _cmu418_vmove_float(result, ones, maskAll); // result = 1.f
        ...
        while (1) { 
			...
			// mask vmove
            __cmu418_mask mask_lowest_bit = _cmu418_init_ones(0);
			for (int i =0 ; i < VECTOR_WIDTH; i++) {
				mask_lowest_bit.value[i] = mask_true.value[i] ? int_lowest_bit.value[i]: mask_lowest_bit.value[i];
			}
