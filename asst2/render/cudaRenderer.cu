...
__device__ void newShadePixel(int circleIndex, float2 pixelCenterNorm, float4 *imagePtr)
{
    float3 rgb;
    float alpha;
    ...
    float3 p = make_float3(cuConstRendererParams.position[index3], cuConstRendererParams.position[index3+1], cuConstRendererParams.position[index3+2]);
...
__device__ int isInCircle(const float2 &pixelCenterNorm, const float3 &circlePos) {
...
__global__ void newkernelRenderPixels(int *gridCirsPtr, uint *gridThreadCirsPtr) {
    int localTidx = threadIdx.y * blockDim.x + threadIdx.x;


    __shared__ uint tidxNumCirs[256];
    ...
    ///
    int circleRange = (cuConstRendererParams.numCircles + (blockDim.x * blockDim.y) - 1) / (blockDim.x * blockDim.y);
    
    int firstCircleIndex = localTidx * circleRange;
    
    int lastCircleIndex = firstCircleIndex + circleRange;
    lastCircleIndex = min(lastCircleIndex, cuConstRendererParams.numCircles);

    // how many circles in this thread
    uint nrCircleThread = 0;


    uint *cirIdxs = gridThreadCirsPtr + localTidx * circleRange + (blockIdx.x + gridDim.x * blockIdx.y) * 100001;

    for (int circle = 0; circle < circleRange; circle++) {
        int globalCirIdx = firstCircleIndex + circle;
        if (globalCirIdx >= cuConstRendererParams.numCircles)
            break;
        int index3 = globalCirIdx * 3;
        float3 circlePos = make_float3( cuConstRendererParams.position[index3],
                                        cuConstRendererParams.position[index3+1],
                                        cuConstRendererParams.radius[globalCirIdx]);
        ...
        if (circleMaxY < gridMinY)
            continue;
        cirIdxs[nrCircleThread++] = globalCirIdx;
    }


    __shared__ uint tidxNumCirsPrefixSum[256];
    __shared__ volatile uint prefixSumArr[256];

    tidxNumCirs[localTidx] = nrCircleThread;
    sharedMemExclusiveScan(localTidx, tidxNumCirs, tidxNumCirsPrefixSum, prefixSumArr, 256);
    __syncthreads();
    
    __shared__ int nrCirGrid;
    if (localTidx == blockDim.x * blockDim.y - 1) {
        nrCirGrid = tidxNumCirsPrefixSum[localTidx] + nrCircleThread;
    }
    __syncthreads();
    int *gridCirIdx = gridCirsPtr + (blockIdx.x + gridDim.x * blockIdx.y) * 100001;
    ...
    for (int localIdx = 0; localIdx < nrCirGrid; localIdx++) {
        ...
        float3 circlePos = make_float3(
                        cuConstRendererParams.position[3 * circleIdx], cuConstRendererParams.position[3 * circleIdx + 1], cuConstRendererParams.radius[circleIdx]);
        if (isInCircle(pixelCenterNorm, circlePos)) {
            float4 *imagePtr = (float4 *)(&cuConstRendererParams.imageData[4 * (indexY * imageWidth + indexX)]);
            newShadePixel(circleIdx, pixelCenterNorm, imagePtr);
        }
    }
    return ;
...
CudaRenderer::render() {
    ...
    int numOfGrids = ((imageWidth + 16 -1) / 16) * ((imageHeight + 16 -1) / 16);

    uint *gridThreadCirsPtr = nullptr;
    int *gridCirsPtr = nullptr;
    cudaMalloc(&gridCirsPtr, sizeof(int) * 100001 * numOfGrids);
    cudaMalloc(&gridThreadCirsPtr, sizeof(uint) * 100001 * numOfGrids);
    newkernelRenderPixels<<<gridDim, blockDim>>>(gridCirsPtr, gridThreadCirsPtr);
    cudaDeviceSynchronize();
    cudaFree(gridCirsPtr);
    cudaFree(gridThreadCirsPtr);
